...
describe('Studio document', () => {
  ...
  it('loads the bundled Studio snapshot when no local project exists', () => {
    ...
    expect(document.library.avatars).toHaveLength(2)
    ...
    expect(document.library.avatars[0].renderStyle).toEqual({ type: 'narra-face' })
    expect(document.library.avatars[1].renderStyle).toEqual({
      type: 'narra-face',
      accessory: 'headset',
    })
    expect(document.library.avatars[0].colors).toEqual({ body: '#a1a1a1', eyes: '#a1a1a1' })
    expect(document.library.avatars[0].body.nodes).toHaveLength(0)
  ...
  it('clears only the persisted Studio project', () => {
    ...
    expect(removeItem).toHaveBeenCalledExactlyOnceWith('narra-face-lab-v1')
  })
  ...
  it('adds the headset variant to an existing saved Memento project', () => {
    const fallback = loadStudioDocument(storage())
    const legacy = structuredClone(fallback)
    legacy.library.avatars = [{ ...legacy.library.avatars[0], name: 'Narra Face' }]
    ...
    expect(document.library.activeAvatarId).toBe('narra-face')
  })
  ...
  it('removes retired upstream avatars from a locally saved project', () => {
    const fallback = loadStudioDocument(storage())
