...
export function StudioInspector({ controller }: { controller: StudioController }) {
  ...
  const mementoVariants = avatars.filter(avatar =>
    ['narra-face', 'narra-face-headset'].includes(avatar.id)
  )
  const showMementoVariants =
    mementoVariants.length > 1 &&
    mementoVariants.some(avatar => avatar.id === activeAvatarId) &&
    mode !== 'photo'
      ...
      >
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            ...
            {!editorPageOpen && showMementoVariants && (
              <nav className="memento-variant-switcher" aria-label={t('Versions de Memento')}>
                {mementoVariants.map(avatar => {
                  const selected = avatar.id === activeAvatarId
                  return (
                    <Button
                      className="memento-variant-option"
                      variant="outline"
                      type="button"
            ...
            {!sequenceEditing && !editing && !bodyEditing && mode === 'export' && (
              <Accordion className="export-panel" defaultValue={['project']}>
                {activeAvatar.renderStyle.type !== 'narra-face' && (
                  <ExportSection
                    value="avatar"
                      ...
                      <div className="runtime-quick-start-step">
                        ...
                        <code>
                          <HighlightedRuntimeCode>
                            {exportFormat === 'react'
                              ? reactQuickStartInstall
                              : webQuickStartInstall}
                          </HighlightedRuntimeCode>
                        </code>
                      </div>
