...
import { renderNarraEyeEditor } from '@/features/rendering/narraFaceGeometry'
...
export function AvatarCanvas({
  ...
  >(null)
  const narraFace = renderStyle.type === 'narra-face'
  const drag = useRef<
    | {
        type: 'arcball'
        startPoint: readonly [number, number]
  ...
  const editor =
    selectedSide === null
      ? null
      : narraFace
        ? renderNarraEyeEditor(poseWithAvatarEyes(expression, avatarEyes), selectedSide)
        : renderEyeEditor(poseWithAvatarEyes(expression, avatarEyes), surface, selectedSide)
  const selectedBodyPath = (() => {
    if (narraFace || !bodyEditing || !selectedBodyNodeId) return null
  ...
  const startHandle = (
    ...
    const liveEditor = narraFace
      ? renderNarraEyeEditor(livePose, selectedSide)
      : renderEyeEditor(livePose, surface, selectedSide)
    const leftEditor = narraFace
      ? renderNarraEyeEditor(livePose, -1)
      : renderEyeEditor(livePose, surface, -1)
    const rightEditor = narraFace
      ? renderNarraEyeEditor(livePose, 1)
      : renderEyeEditor(livePose, surface, 1)
    drag.current = {
      type,
  ...
  return (
    <div
      className={`avatar-wrap${renderStyle.type === 'pixel' ? ' is-pixel-rendered' : ''}${narraFace ? ' is-narra-face' : ''}`}
    >
      {playback && (
        <motion.div
      ...
      <svg
        ...
        aria-label={
          narraFace
            ? renderStyle.accessory === 'headset'
              ? 'Memento with headphones'
              : 'Memento'
            : t('Avatar procédural')
        }
        onPointerMove={move}
      ...
      >
        {narraFace ? (
          <motion.g style={{ x: offsetX, y: offsetY }}>
            {backPaths.map((pathValue, index) => (
              <SvgTransformPath
                className={`narra-face-path narra-face-structure narra-face-headset ${highlight === 'head' ? 'cyan-outline' : ''}`}
                d={pathValue}
            ...
            <SvgTransformPath
              className={`narra-face-path narra-face-structure ${highlight === 'head' ? 'cyan-outline' : ''}`}
              d={headPath}
              svgTransform={headTransform}
              onPointerDown={startDrag}
            />
            <SvgTransformPath
              className={`narra-face-path narra-face-eye ${selectedSide === -1 || highlight === 'left' || highlight === 'both' ? 'cyan-outline' : ''}`}
              d={leftPath}
            ...
            />
            <SvgTransformPath
              className={`narra-face-path narra-face-eye ${selectedSide === 1 || highlight === 'right' || highlight === 'both' ? 'cyan-outline' : ''}`}
              d={rightPath}
            ...
            <motion.g style={{ x: offsetX, y: offsetY }}>
              {backPaths.map((pathValue, index) => (
                <motion.path
                  ...
                  d={pathValue}
                  transform={backTransforms[index]}
                  key={index}
              ...
              {frontPaths.map((pathValue, index) => (
                <motion.path
                  className={`avatar-head ${highlight === 'head' ? 'cyan-outline' : ''}`}
                  d={pathValue}
                  transform={frontTransforms[index]}
                  key={index}
        ...
        {!narraFace && bodyEditing && selectedBodyNode && (
          <BodyNodeGizmo
            svgRef={svgRef}
