    ...
    renderStyle.type === 'narra-face'
      ? [
          ...backPaths.map((value, index) =>
            path(value, colors.body, 0.58, scene.backTransforms[index].get())
          ),
          path(headPath, colors.body, 1, scene.headTransform.get()),
          path(
            scene.leftPath.get(),
            colors.eyes,
            scene.leftOpacity.get(),
            scene.leftTransform.get()
          ),
          path(
            scene.rightPath.get(),
            colors.eyes,
            scene.rightOpacity.get(),
            scene.rightTransform.get()
          ),
          ...frontPaths.map((value, index) =>
            path(value, colors.body, 1, scene.frontTransforms[index].get())
          ),
        ].join('')
      : [
