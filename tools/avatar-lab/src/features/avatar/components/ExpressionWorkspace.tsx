...
export function ExpressionPreview({
  ...
  const outlineWidth = avatarBodyOutlineWidth()
  if (geometry.pathTransforms?.kind === 'narra-face') {
    return (
      <svg
        className="avatar-preview narra-face-preview"
        viewBox="-150 -150 300 300"
        aria-hidden="true"
      >
        {geometry.backPaths.map((pathValue, index) => (
          <path
            className="narra-face-headset"
            d={pathValue}
            transform={geometry.pathTransforms?.back[index]}
            fill={resolvedColors.body}
            key={`narra-back-${index}`}
          />
        ))}
        <path
          d={geometry.headPath}
