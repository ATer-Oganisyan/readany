 ...
 * план, не добавлять персонажей и текст. Для портрета героя — своё «живое
 * фото»: дыхание, моргание, лёгкий поворот головы.
 */
...
const MOTION_GUARDRAILS =
  ...
  "без резких смен плана, без появления новых персонажей, без текста.";
...
const PORTRAIT_MOTION =
  "Лёгкое живое движение: дыхание, моргание, лёгкий поворот головы, ветер в волосах; " +
  "персонаж НЕ говорит, взгляд остаётся на зрителе, стиль сохраняется.";

/** Промпт «живого фото» портрета героя: дыхание и моргание, без речи. */
export function buildPortraitMotionPrompt(): string {
  return `${PORTRAIT_MOTION} ${MOTION_GUARDRAILS}`;
}
