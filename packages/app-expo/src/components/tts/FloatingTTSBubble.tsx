...
export function FloatingTTSBubble() {
  ...
  const bubbleVisible = isActive;

  useEffect(() => {
    if (!bubbleVisible) setShowPlayer(false);
  }, [bubbleVisible]);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: position and visibility changes intentionally trigger a fresh native measurement
  useEffect(() => {
    if (!bubbleVisible) {
    ...
    measureBubble();
  }, [bubbleOffset.x, bubbleOffset.y, bubbleVisible, measureBubble, showPlayer]);
