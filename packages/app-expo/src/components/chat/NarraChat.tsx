...
export function NarraChat({
  ...
  const chatTheme = resolvedScheme === "dark" ? defaultDarkTheme : defaultLightTheme;
  ...
  const renderAccessory = useCallback(() => {
    if (!quotes.length && !deepThinking && !spoilerFree) return null;
    return (
      <View
        style={[
          styles.accessory,
  ...
  }, [chatTheme, deepThinking, onRemoveQuote, quotes, resolvedScheme, spoilerFree]);
...
function AccessoryRow({
  ...
  const colors = dark ? defaultDarkTheme.colors : defaultLightTheme.colors;
  return (
