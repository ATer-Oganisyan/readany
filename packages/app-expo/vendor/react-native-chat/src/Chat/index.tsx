...
function Chat<TMessage extends IMessage = IMessage> (
  ...
  const systemColorSchemeValue = useColorScheme()
  // React Native 0.83 may report `unspecified`; the upstream 4.2 branch only
  // models light/dark/null. Treat it as no preference and use the light base.
  const systemColorScheme = systemColorSchemeValue === 'dark' || systemColorSchemeValue === 'light'
    ? systemColorSchemeValue
    : null
...
function ChatWrapper<TMessage extends IMessage = IMessage> (props: ChatProps<TMessage>) {
  const {
    keyboardProviderProps: _keyboardProviderProps,
    disableKeyboardProvider: _disableKeyboardProvider = false,
    ...rest
  } = props
