(async () => {
 ...
 gateway.setNarraGatewayFetch((url,init)=>{
   if(String(url).endsWith('/v2/speech/synthesize')&&String(init?.body).includes('NARRA_TTS_LOCAL_FIXTURE')){
     routed++;return expoFetch('http://127.0.0.1:8697/slow',{method:'POST',headers:{'content-type':'application/json'},body:init.body,signal:init.signal});
   }
   return expoFetch(url,init);
 });
 const directory=fs.documentDirectory+'narra-media';
 const before=new Set(await fs.readDirectoryAsync(directory));
 try {
   await player.speak(['NARRA_TTS_LOCAL_FIXTURE cancel one.','NARRA_TTS_LOCAL_FIXTURE cancel two.'],{...tts.getState().config,engine:'edge',rate:1});
   await pause(250);player.stop();await pause(1800);
   const after=await fs.readDirectoryAsync(directory);
   result.requests=routed;result.newFiles=after.filter(name=>!before.has(name)&&name.startsWith('speech-'));
   result.queueAfterStop=(await tp.getQueue()).length;result.states=states;result.chunks=chunks;
 } finally {
   player.stop();gateway.setNarraGatewayFetch(expoFetch);result.fetchRestored=true;
 }
 if(result.requests!==2||result.newFiles.length||result.queueAfterStop||result.chunks.length)throw Error(JSON.stringify(result));
 return result;
})()
