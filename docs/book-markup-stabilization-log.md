...
|---:|---|---|---|---|
| 0 | Локальный Docker-контур, review UI, frozen 4-book inputs | Сделать результат наблюдаемым и повторяемым до правки prompt | В процессе локальной проверки | Не менять алгоритм до baseline |
