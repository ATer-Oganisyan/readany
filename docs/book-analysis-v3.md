...
Текущая версия `character-profile-v1` проверяет ответ модели целиком. Если хотя
бы одно поле профиля ссылается на неизвестный evidence ID или на evidence
несовместимого типа, весь профиль отклоняется и job повторяется. Например,
`traits` разрешено подтверждать только наблюдениями `character_trait`, а не
`character_action`. Это fail-closed поведение сохраняет доказательность, но один
плохой claim пока может уничтожить остальные корректные поля профиля.
...
- Поэлементная фильтрация реализована для `scan`, но ещё не для
  `character-profile-v1`: один несовместимый claim отклоняет весь профиль.
- Resolver объединяет только одинаковые нормализованные имена и явные
  `character_alias`. Поэтому варианты вроде полного имени, имени с фамилией и
  одной фамилии могут остаться отдельными персонажами, если модель не создала
  явное alias-наблюдение.
- Повторные описательные обозначения, например «молодой человек» или «мещанин»,
  могут получить статус `confirmed` и отдельное synthesize-задание. До публичного
  использования нужен отдельный консервативный фильтр именных и описательных
  кандидатов.
