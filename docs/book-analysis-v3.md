...
| --- | --- | --- |
| Pipeline | `book-analysis-v17` | Малые scan-фрагменты, адаптивный fallback, evidence-aware resolver и семантический контроль полноты |
...
## 32. `book-analysis-v17`: отделение роли человека от именованной группы

Дополнительный аудит той же выборки показал, что одного наличия заглавного слова
внутри фразы недостаточно: «племя Земзе», «статуя Магацитла» и «студенты
Хирургической академии» всё ещё выглядели именованными сущностями.

В v17 строчная многословная форма подтверждается как персонаж только с известным
человеческим титулом или ролью перед собственным именем. Коллективы и явно
неодушевлённые носители остаются evidence-backed кандидатами. При этом формы
«коллежский асессор Ковалев», «дядя Козий Зоб» и «председатель Цекубу» сохраняют
возможность подтверждения.

## Известный технический долг

### `TD-BA-001`: персонажи из front matter считаются персонажами истории

**Статус:** открыт. **Приоритет:** высокий для provisional-разметки.

Pipeline исключает случай, когда имя автора дословно скопировано с титульной
страницы, но не отделяет критическое предисловие, посвящение, список иллюстраций
и другие front matter-диапазоны от повествования. Поэтому доказанное упоминание
человека в предисловии получает обычный `character_*` claim, а его координата
становится `firstAppearanceTextOffset`. Ошибка затрагивает и промежуточный
список во время scan, и потенциальную финальную публикацию.

Зафиксированный пример на TEST 17 августа 2026 года:

- книга `Pride and Prejudice`, edition
  `5413a931-7285-42f1-8c6d-20a765a178bc`, run
  `bc60e6a6-9527-4bd3-a3b6-e8213fb8dc91`;
- позиция читателя соответствовала offset `18 300`, а текст первой главы
  начинался примерно с offset `29 932`;
- manifest уже отдавал 21 provisional-персонажа;
- Fanny, Edmund, Mary Crawford, Mrs. Norris, Miss Bates, Mrs. Rushworth,
  Mrs. Musgrove и John Dashwood были извлечены из критического предисловия о
  других романах Джейн Остин;
- формы `Darcy` и `Mr. Darcy` временно отображались как разные персонажи.

Большое значение scan progress в этом случае корректно: chunks анализируются
параллельно по всей книге. Некорректна именно доступность персонажей по раннему
evidence из front matter; она не означает, что читатель дошёл до их появления в
сюжете.

Требуемое исправление:

1. На prepare/scan классифицировать диапазоны как narrative, front matter и
   back matter, используя структуру EPUB и консервативные текстовые признаки.
2. Не использовать non-narrative evidence для `firstAppearanceTextOffset`,
   reader unlock и создания персонажа истории. При необходимости сохранять его
   только как диагностический контекст.
3. Не показывать provisional-сущность, пока её существование не подтверждено
   narrative evidence; схлопывать титулованные формы до выдачи в manifest.
4. Повысить версии pipeline/prompt/extractor, выполнить новый run и не
   переиспользовать прежние cache keys.
5. Добавить regression fixture с предисловием, которое упоминает персонажей
   текущей и других книг того же автора.

Критерии приёмки на зафиксированном EPUB:

- до начала первой главы персонажи из критического предисловия не доступны;
- Fanny, Edmund, Mary Crawford, Miss Bates, Mrs. Musgrove и John Dashwood не
  входят в персонажей `Pride and Prejudice`;
- `Darcy` и `Mr. Darcy` не появляются одновременно как две reader-visible
  сущности;
- provisional и финальный manifest применяют одинаковую narrative-границу;
- доказательную схему `personality`, `character_trait` и `appearance` эта
  работа не меняет.
