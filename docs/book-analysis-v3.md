...
| --- | --- | --- |
| Pipeline | `book-analysis-v12` | Малые scan-фрагменты, resolver, барьеры и семантический контроль полноты |
| Scan prompt и extractor | `book-scan-v7` | Quote-only evidence с серверным определением координат и безопасной диагностикой |
...
## 6. Доказательная валидация `book-scan-v7`
...
2. Выполняется точный посимвольный поиск всех вхождений `quote` в context.
3. Из найденных вхождений учитываются только те, начало которых находится в core.
4. Если внутри core осталось ровно одно вхождение, сервер вычисляет координаты.
5. Если цитата отсутствует или неоднозначна внутри core, наблюдение
   отбрасывается.
...
`book-scan-v7` сохраняет доказанные элементы и отбрасывает только проблемные.
...
Наблюдение `relationship` разрешено только для отношений между персонажами, и
каждый его участник обязан иметь отдельное `character_*`-наблюдение в том же
ответе. Неполный ответ не кэшируется и повторяется обычной политикой scan-job.
