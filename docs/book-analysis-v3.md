...
| --- | --- | --- |
| Pipeline | `book-analysis-v13` | Малые scan-фрагменты, адаптивный fallback, resolver и семантический контроль полноты |
| Scan prompt и extractor | `book-scan-v8` | Quote-only evidence, серверные координаты и адаптивное деление проблемного core |
...
## 6. Доказательная валидация `book-scan-v8`
...
`book-scan-v8` сохраняет доказанные элементы и отбрасывает только проблемные.
