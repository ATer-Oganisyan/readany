...
| --- | --- | --- |
| Pipeline | `book-analysis-v5` | Стадии, resolver, барьеры и политика пустого scan |
...
- Список описательных обозначений консервативен и конечен; неизвестная роль с
  заглавной буквы всё ещё может быть принята за имя и требует shadow-контроля.
- Автоматическое объединение частей ФИО не использует морфологию и не склеивает
  одно имя с полным именем. Это снижает риск объединения разных людей, но может
  оставить часть алиасов отдельными сущностями.
...
`book-analysis-v5` / `character-profile-v2`; их результат должен быть
