...
| --- | --- | --- |
| Pipeline | `book-analysis-v11` | Малые scan-фрагменты, resolver, барьеры и семантический контроль полноты |
| Scan prompt и extractor | `book-scan-v6` | Извлечение, фильтрация метаданных, отношений и чрезмерно потерянных ответов |
...
## 6. Доказательная валидация `book-scan-v6`
...
`book-scan-v6` сохраняет доказанные элементы и отбрасывает только проблемные.
