...
финальной передачей он исправлен в коммите `f44ec6c1`:

- `openBook` получает `sourceId`, равный пути EPUB-снимка;
- каждое событие `relocate` возвращает тот же `sourceId`;
- состояние чанков хранится отдельно для каждого пути снимка;
- событие старого снимка рассчитывается по его собственному состоянию;
- после изменения `book.filePath` запоздалые события старого снимка
  игнорируются.

Повторная проверка `f44ec6c1` на чистой установке:

- «Деревня» открылась после первого чанка, составлявшего примерно 15,3% текста;
- следующий чанк запросился после `localFraction > 0.60`;
- открылся новый путь `*-catalog-131071.epub` (граница UTF-8 может быть меньше
  ровно 128 KiB);
- абсолютный прогресс перед переключением был примерно 9,88%, после
  восстановления CFI — примерно 9,51%;
- прежнего ошибочного скачка прогресса до удвоенного значения больше нет.

## Проверки и критерии приёмки

Автотесты после исправления:
