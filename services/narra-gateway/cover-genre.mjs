...
const GENRE_RULES = [
  ...
  },
  {
    ...
    content:
      /(scientific|technology|programming|научн.*популяр|технологическ|программировани)/iu
  },
  {
    profile: {
      id: 'history-politics',
...
export function resolveCoverGenreProfile({ subjects = [], title, description, excerpt }) {
  const metadata = subjects.join(' ').normalize('NFKC')
  if (metadata) {
    const metadataMatch = GENRE_RULES.find((rule) => rule.metadata.test(metadata))
    if (metadataMatch) return metadataMatch.profile
  }

  const content = [title, description, excerpt]
    .filter(Boolean)
    .join(' ')
    .normalize('NFKC')
    .slice(0, 6_000)
