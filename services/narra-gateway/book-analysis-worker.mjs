...
const workerId = String(
  process.env.BOOK_ANALYSIS_WORKER_ID || `book-analysis-${randomUUID()}`
)
const pollMs = parseEnvInt(process.env, 'BOOK_ANALYSIS_WORKER_POLL_MS', 1_000, 60_000)
const leaseSeconds = parseEnvInt(
  process.env,
  'BOOK_ANALYSIS_JOB_LEASE_SECONDS',
  300,
  3_600
)
const leaseRenewMs = parseEnvInt(
  process.env,
  'BOOK_ANALYSIS_LEASE_RENEW_MS',
  60_000,
  1_800_000
)
const idleLogMs = parseEnvInt(
  process.env,
  'BOOK_ANALYSIS_IDLE_LOG_MS',
  300_000,
  3_600_000
)
    ...
    try {
      ...
      if (result.status === 'idle') {
        if (Date.now() - lastIdleLogAt >= idleLogMs) {
          log.info('worker.idle', 'Очередь подготовки книг пуста', { worker: workerId })
          lastIdleLogAt = Date.now()
        }
        await delay(pollMs, shutdown.signal)
      }
    } catch (error) {
      log.error('worker.queue_unavailable', 'Очередь анализа временно недоступна', {
        worker: workerId,
