...
docker exec "$CURRENT_CONTAINER" env | awk -F= '
  $1 ~ /^(AI_|ALLOW_|ANALYTICS_|API_LIMIT_|CORS_|EVENT_|GATEWAY_|IMAGE_|IMPORT_|INSTALLATION_|KANDINSKY_|LLM_|OPENROUTER_|REFRESH_|REGISTRATION_|SALUTE|SBER_|SPEECH_|VIDEO_)/ &&
  $1 !~ /^TRACTION_/ { print }
' > "$temporary"
